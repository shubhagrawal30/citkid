...
class CRS:
    ...
    async def write_tones(self, module_index, fres, ares):
        """
        ...
        await self.d.clear_channels(module = module_index)
        
        nco = await self.d.get_nco_frequency(self.d.UNITS.HZ, module=module_index)
        async with self.d.tuber_context() as ctx:
            for ch, (fr, ar) in enumerate(zip(fres,ares_amplitude)):
                ## To be wrapped in context_manager
                ctx.set_frequency(fr-nco, self.d.UNITS.HZ, ch+1, module=module_index)
                ctx.set_amplitude(ar, self.d.UNITS.NORMALIZED, target=self.d.TARGET.DAC, channel=ch+1, module=module_index)
            await ctx()

    async def sweep_sequential(self, module_index, ncos, frequencies, ares, nsamps = 10, verbose = True, 
        ...
        for nco_index, nco_freq in self.nco_freq_dict.items():
            await self.set_nco(module_index, nco_freq, verbose = verbose)
            desc = pbar_description + f' {nco_index} / {len(self.nco_freq_dict)}'
            fi, zi = await self.sweep(module_index, self.frequencies_dict[nco_index], 
    ...
    async def sweep(self, module_index, frequencies, ares, nsamps = 10, verbose = True, pbar_description = 'Sweeping',
                    return_raw = False):
        """
        ...
        await self.write_tones(module_index, fres, ares)
        # Initialize z array 
        ...
        for sweep_index in pbar:
            ...
            async with self.d.tuber_context() as ctx:
                for ch in range(n_channels):
                    ...
                    ctx.set_frequency(f - self.nco_freq, self.d.UNITS.HZ, ch + 1, module = module_index)
                await ctx()

            # take data and loopback calibration data
            await self.d.set_dmfd_routing(self.d.ROUTING.CARRIER, module_index)
            samples_cal = await self.d.get_samples(21,module=module_index)
            await self.d.set_dmfd_routing(self.d.ROUTING.ADC, module_index)
            samples = await self.d.get_samples(nsamps + 1,module=module_index)
    ...
    async def sweep_linear(self, module_index, fres, ares, bw = 20e3, npoints = 10, 
        ...
        z = await self.sweep(module_index, f, ares, nsamps = nsamps, 
                            verbose = verbose, pbar_description = pbar_description)
        return f, z
    
    async def sweep_linear_sequential(self, module_index, ncos, fres, ares, bw = 20e3, npoints = 10, 
                           nsamps = 10, verbose = True, pbar_description = 'Sweeping'):
        """
        Performs a frequency sweep where each channel is swept over the same range in 
        frequencies
        ...
        fres, ares = np.asarray(fres), np.asarray(ares)
        f = np.linspace(fres - bw / 2, fres + bw / 2, npoints).T
        z = await self.sweep_sequential(module_index, ncos, f, ares, nsamps = nsamps, 
                            verbose = verbose, pbar_description = pbar_description)
        return f, z

    async def sweep_qres(self, module_index, fres, ares, qres, npoints = 10, nsamps = 10,
    ...
    async def sweep_qres_sequential(self, module_index, ncos, fres, ares, qres, npoints = 10, nsamps = 10,
                         verbose = True, pbar_description = 'Sweeping'):
        """
        Performs a frequency sweep where the span around each frequency is set 
        equal to fres / qres 
    ...
    async def sweep_full(self, module_index, amplitude, npoints = 10, 
                         nsamps = 10, verbose = True, pbar_description = 'Sweeping'):
        """
        ...
        bw = 600e6 / 1024 + 200
        f, z = await self.sweep_linear(module_index, fres, ares, bw = bw, npoints = npoints,
    ...
    async def sweep_full_sequential(self, module_index, ncos, amplitude, npoints = 10, 
                         nsamps = 10, verbose = True, pbar_description = 'Sweeping'):
        """
        Performs a frequency sweep over the full 600 MHz bandwidth around the NCO 
        frequency
        ...
        ares = amplitude * np.ones(len(fres))
        bw = 600e6 / 1024 + 200
        f, z = await self.sweep_linear_sequential(module_index, ncos, fres, ares, bw = bw, npoints = npoints,
                                      nsamps = nsamps, verbose = verbose, 
                                      pbar_description = pbar_description)
        f, z = f.flatten(), z.flatten()
        ix = np.argsort(f)
    ...
    async def capture_noise(self, module_index, fres, ares, noise_time, fir_stage = 6,
        ...
        parser = subprocess.Popen([parser_loc, '-d', data_path, '-i', interface, '-s', 
                                   f'{self.crs_sn:04d}', '-m', str(module_index), '-n', str(num_samps)], 
                                   shell=False)
        ...
        zraw = convert_parser_to_z(data_path, self.crs_sn, module_index, ntones = len(fres))
    ...
    async def capture_fast_noise(self, module_index, frequency, amplitude, nsamps):
        """ 
        ...
        """
        fsample = 625e6 / 256
        await self.write_tones(module_index, [frequency], [amplitude])
        await self.d.set_dmfd_routing(self.d.ROUTING.ADC, module_index)
        sleep(1)
